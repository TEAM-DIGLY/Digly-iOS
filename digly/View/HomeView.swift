...
struct HomeView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack (spacing:4) {
                ...
                Image("\(appState.userDigly.name)Logo")
                    .resizable()
                ...
                Image("alert")
                Image("\(appState.userDigly.name)Profile")
            }
            .frame(height:80)
            ...
            ZStack{
                if viewModel.remainingDate > 4 {
                    Image("\(appState.userDigly.name)Box")
                    ...
                    Image("\(appState.userDigly.name)LiveBox")
                        .aspectRatio(contentMode: .fit)
                        .frame(width: 300, height: 300)
                            ...
                            Text("나의 0번째 문화생활")
                                ...
                                .foregroundStyle(viewModel.remainingDate > 3 ?
                                    .neutral55 : viewModel.remainingDate > 0 ?
                                                 appState.userDigly.lightColor : .opacityCool55)
            ...
            HStack(alignment: .bottom) {
                // My Notes
                VStack{
                    Image("\(appState.userDigly.name)Avatar")
                        .padding(.bottom,-28)
    ...
    private func backgroundColorForRemainingDays(_ days: Int) -> Color {
        ...
        case 4:
            return appState.userDigly.lightColor
        default:
            return .neutral85
        }
    }
