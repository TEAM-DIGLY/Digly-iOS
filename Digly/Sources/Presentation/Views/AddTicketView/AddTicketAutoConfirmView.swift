...
struct AddTicketAutoConfirmView: View {
    @StateObject private var viewModel: AddTicketAutoConfirmViewModel
    ...
    let onNavigatedToEnd: (Ticket) -> Void
    
    init(
        ticketFormData: CreateTicketFormData,
        onNavigatedToEnd: @escaping (Ticket) -> Void
    ) {
        self._viewModel = StateObject(wrappedValue: AddTicketAutoConfirmViewModel(ticketFormData: ticketFormData))
        self.onNavigatedToEnd = onNavigatedToEnd
    }
    
    var body: some View {
        DGScreen(backgroundColor: .common0, isLoading: viewModel.isLoading, onClick: { isFocused = false }) {
            VStack(spacing: 0) {
                TitleBackNavBar(title: "티켓 추가하기", isDarkMode: true) {
                    Button(action: {
                        viewModel.submitTicket { ticket in
                            onNavigatedToEnd(ticket)
                        }
                    }) {
                        Text("완료")
                            .fontStyle(.headline2)
...
// MARK: - Components
extension AddTicketAutoConfirmView {
    @ViewBuilder
